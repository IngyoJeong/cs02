...
namespace _020_FormClock // 4/5 아날로그 시계에 숫자판과 디지털 시계, 끝내기 추가
{
    public partial class Form1 : Form
    {
        ...
        const int clientSize = 300; //상수로 클라이언트 크기를 지정
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            ...
            if(aClockFlag == true) //아날로그
            {
                ...
                //시계바늘을 그린다. (핵심, 각도를 계산)
                double radHr = (c.Hour % 12 + c.Minute / 60) * 30 * Math.PI / 180; //시간당 60도
            ...
            else //디지털
            {
                ...
                label1.Location = new Point(ClientSize.Width / 2 - label1.Width / 2,
                    ClientSize.Height / 2 - label1.Height / 2); //ClientSize -> panel1
            }
        }

        //각도를 받아서 시계바늘 그리는 메서드
        ...
        private void DrawClockFace()
        {
            ...
            for (int d = 0; d < 360; d+= 30)
            {
                ...
                int y = (int)(center.X - radius * Math.Cos(d * Math.PI / 180));
                g.FillEllipse(Brushes.AliceBlue, new Rectangle(x - dotSize / 2, y - dotSize / 2,
                    dotSize, dotSize));
            }
        }
